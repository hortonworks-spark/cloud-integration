...
import com.cloudera.spark.cloud.s3.audit.{LogParser, S3LogRecordParser, ServerLogEntry}
import com.cloudera.spark.cloud.test.UnitTestSuite

/**
 * log parsing
 */
class TestLogParsing extends UnitTestSuite {
  ...
  private def parse(r: String): ServerLogEntry = {
    logInfo(s"parsing [$r]")
    val p = LogParser.parse(r)
    assert(p.isDefined, s"Failed to parse $r")
    val log = p.get
    logInfo(s"$log")
    log
  }
