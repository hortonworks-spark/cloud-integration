...
package org.apache.spark.streaming.hortonworks
...
/**
 * This class represents an input stream that monitors an object store via the
...
class CloudInputDStream[K, V, F <: NewInputFormat[K, V]](
  ...
  private val serializableConfOpt = conf.map(new ConfigSerDeser(_))
  ...
  private val minRememberDurationS = {
    Seconds(_ssc.sparkContext.conf
      .getTimeAsSeconds("spark.streaming.fileStream.minRememberDuration",
        _ssc.sparkContext.conf.get("spark.streaming.minRememberDuration", "60s")))
  }
  ...
  private val initialModTimeIgnoreThreshold = if (newFilesOnly) clock
    .getTimeMillis() else 0L
  ...
  private def filesToRDD(files: Seq[String]): RDD[(K, V)] = {
    val fileRDDs = files.map { file =>
      val rdd = serializableConfOpt.map(_.get()) match {
  ...
  private def fs: FileSystem = {
    if (_fs == null) _fs = directoryPath
      .getFileSystem(ssc.sparkContext.hadoopConfiguration)
    _fs
  }
  ...
  private def readObject(ois: ObjectInputStream): Unit = Utils
    .tryOrIOException {
  ...
  class FileInputDStreamCheckpointData extends DStreamCheckpointData(this) {
    ...
    override def update(time: Time) {
      ...
      batchTimeToSelectedFiles
        .synchronized {hadoopFiles ++= batchTimeToSelectedFiles}
    }
      ...
      hadoopFiles.toSeq.sortBy(_._1)(Time.ordering).foreach {
        case (t, f) =>
          ...
          logInfo("Restoring files for time " + t + " - " +
            f.mkString("[", ", ", "]"))
          batchTimeToSelectedFiles
            .synchronized {batchTimeToSelectedFiles += ((t, f))}
...
object FileInputDStream {
  ...
  def defaultFilter(path: Path): Boolean =
    !path.getName().startsWith(".") && !path.getName.startsWith("_")
