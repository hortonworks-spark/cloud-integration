...
/**
  ...
  * Configuration.
  */
trait HConf extends CloudLogging {
  /**
  ...
  /**
    * Dump all config options with the prefix.
    *
    * @param conf   configuration
    * @param prefix prefix
    * @return the count
    */
  protected def dumpConfigOptions(conf: Configuration, prefix: String): Int = {
    val count = conf.getPropsWithPrefix(prefix).toMap.map {
      p =>
        val key = prefix + p._1
        val value = p._2
        val sources = conf.getPropertySources(key)
        val l = if (sources != null) sources.mkString("[", ",", "]") else "[]"
        info(s"'$key' -> '$value' source $l")
        1
    }.size
    info(s"$count conf option(s) with prefix $prefix")
    count
  }
}
