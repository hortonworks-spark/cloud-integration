...
import com.hortonworks.spark.cloud.MinimalStoreExample
...
/**
  ...
  */
class Generator extends MinimalStoreExample {
...
//    applyObjectStoreConfigurationOptions(sparkConf, false)
    ...
    dumpConfigOptions(hadoopConf, "fs.s3a")
    dumpConfigOptions(hadoopConf, "fs.adl")
//    dumpConfigOptions(hadoopConf, "fs.wasb")
    try {
      val suffix = ".txt"
      ...
      val filepaths = monthsByYear.flatMap { case (year, month) =>
        filePerMonthRange.map(ranger =>
          "%1$04d/%1$04d-%2$02d/%1$04d-%2$02d-%3$04d"
            .format(year, month, ranger) + suffix
        )
      ...
      info(s"Destination is $destPath")
      rm(destFS, destPath)
      val parent = destPath.getParent()
      if (parent != null && !parent.isRoot) {
        destFS.mkdirs(parent)
      }
      val configSerDeser = new ConfigSerDeser(hadoopConf)
      ...
      val putDataRDD = filesRDD.map(uri => {
        ...
        val executionTime = time {
          ...
          out.close()
          info(s"File System = $fs")
        }
        (jobDest.toUri, written, crc.getValue, executionTime)
      }).cache()

      info(s"Initial File System state = $destFS")
      ...
      info(s"Time to generate ${filesRDD.count()} entries ${
        toHuman(collectionTime)
      }")
      info(s"Aggregate execution time ${toHuman(aggregatedExecutionTime)}")
      info(s"File System = $destFS")
      ...
      info(s"time to list paths under $destPath: $listDuration")
  ...
  def verifyLength(name: String, expectedFileLength: Long,
    actual: Long): Unit = {
