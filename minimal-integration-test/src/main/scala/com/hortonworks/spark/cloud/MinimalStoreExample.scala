...
import com.hortonworks.spark.cloud.utils.{CloudLogging, HConf, TimeOperations}
 ...
 */
trait MinimalStoreExample extends CloudLogging /*with CloudTestKeys*/ with
TimeOperations with HConf with Serializable {
  ...
  protected def execute(operation: (SparkConf, Array[String]) => Int, args: Array[String]): Unit = {
    ...
    try {
      ...
      case e: Exception =>
        error(s"Failed to execute operation: $e", e)
